    ...
    <div class="container">
        ...
        <div class="details">
            ...
            <div class="info"><strong>Réservation:</strong>
                @if ($data_reservation->service->type == 'Trajet')
                    {{ $data_reservation->service->departure }} -> {{ $data_reservation->service->destination }}
                @endif
                @if ($data_reservation->service->type == 'Location')
                    {{ $data_reservation->label }}
                @endif
            </div>
            <div class="info"><strong>Type de Réservation:</strong> {{ $data_reservation->service->type }}</div>
            ...
            <div class="info"><strong>Prix :</strong> {{ $data_reservation->service->price }} FCFA</div>
        </div>

        <div class="details">
            <h2 class="section-title">Informations Client</h2>
            <div class="info"><strong>Nom du Client:</strong> {{ Auth::user()->name }}</div>
            <div class="info"><strong>Téléphone:</strong> {{ Auth::user()->phone }}</div>
        ...
        <div class="details">
            ...
            @if ($data_reservation->comment)
                <div class="info">{{ $data_reservation->comment }}</div>
            @endif
        </div>

    </div>
    <div class="info"> copiez ce lien, associé le à l'email de confirmation à envoyer au client à l'adresse :
        {{ Auth()->user()->email }} <a class="link_payment"
            href="https://terangashuttle.com/reservation/{{ $data_reservation->id }}"> lien de paiement </a> </div>
    </div>
