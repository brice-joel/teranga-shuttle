...
@section('content')
    <section>
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <form method="POST"
                action="{{ $trajet->id ? route('admin.trajet.update', $trajet) : route('admin.trajet.store') }}"
                class="space-y-4">
                @if ($trajet->id)
                    @method('PUT')
                @endif
                ...
                <div>
                    <label for="duree_moyenne" class="block text-gray-700 text-sm font-bold mb-2">Durée Moyenne
                        (minutes)</label>
                    <input type="number" name="duration" id="duration" value="{{ old('duration', $trajet->duration) }}"
                        required
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
